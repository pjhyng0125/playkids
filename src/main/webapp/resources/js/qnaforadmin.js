 ...
 function connect() {
	    ...
	    sock.onmessage = function(evt) {
    	 var data = evt.data;
    	   ...
    	   if(checkMid == msgResult.from_id && msgResult.message_readTime == undefined){
    		   appendMessage(msgResult);
    		   msgResult.message_readTime=new Date();
    		   sock.send(JSON.stringify(msgResult));
    	   }else if(msgResult.from_id =="manager"){
    		   appendMessage(msgResult);
    	   }
	    };
